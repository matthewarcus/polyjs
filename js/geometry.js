...
function Schwarz(angles) {
    ...
    for (var i = 0; i != regions.length; i++) {
        ...
        var pi = getpoint(Vector.applybary(params.P,px,qx,rx));
        var qi = getpoint(Vector.applybary(params.Q,px,qx,rx));
        var ri = getpoint(Vector.applybary(params.R,px,qx,rx));
    ...
    this.faces = faces;
    this.snuba = getsnub(params.p,params.q,params.r);
    this.describe = function (verbose) {
        ...
        console.log(pfaces.length + " ppoints; " + 
                    qfaces.length + " qpoints; " +
                    rfaces.length + " rpoints");
        for (var i = 0; i < regions.length; i++) {
    ...
    this.tri2bary = function(tri) {
        var p = this.params.p;
        var q = this.params.q;
        var r = this.params.r;
        return Vector.tri2bary(tri,p,q,r);
    };
    this.applybary = function(bary,region) {
        var points = this.points;
        var regions = this.regions;
...
// our application.
function setequal2(s,t,eps)
{
    console.assert(s.length == t.length,
                   "Incompatible set lengths");
    for (var i = 0; i < s.length; i++) {
        var found = false;
...
function makefacedata(schwarz, bary)
{
...
function makefacelines(schwarz,facedata,hideface,centre,type,i)
{
    ...
    for (var type1 = 0; type1 < 3; type1++) {
        ...
        var faces1 = schwarz.faces[type1];
        ...
        for (var j = 0; j < faces1.length; j++) {
            if ((type != type1 || i != j) && faces1[j]) {
                ...
                for (var type2 = 0; type2 < 3; type2++) {
                    ...
                    var faces2 = schwarz.faces[type2];
...
function stellate(schwarz,facedata,hideface)
{
    var points = schwarz.points;
    var regions = schwarz.regions;
    facedata.facelines = [];
